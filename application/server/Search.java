...
class Search {
	...
	private void fun() {
		try {
			...
			HttpURLConnection connection = (HttpURLConnection) url[provider]
				.openConnection();
			...
			if (provider == 0) {
				...
				bw.write(
					"w=" + this.word + "&key=DCC1EAA89FD3B5C540FDA96B080B5F72");
			}
			bw.flush();
	...
	private String arrangeOutput(String result)
		...
		if (provider == 0) {
			...
			output = URLDecoder.decode(tmp[tmp.length - 1].substring(1,
				tmp[tmp.length - 1].length() - 4), "UTF-8");
