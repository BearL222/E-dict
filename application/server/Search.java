...
class Search {
	...
	private String arrangeOutput(String result)
		...
		if (provider == 0) {
			...
			output += "美音："
				+ tmp[3].substring(1, tmp[3].indexOf("phonetic") - 3) + "\n";
			output += "发音："
				+ tmp[4].substring(1, tmp[4].indexOf("uk-phonetic") - 3) + "\n";
			output += "英音："
				+ tmp[5].substring(1, tmp[5].indexOf("explains") - 3) + "\n";
			output += "意思：" + tmp[6].substring(1, tmp[6].indexOf("query") - 4)
				.replace("\"", "") + "\n";
			output += "网络:\n";
			String webVal =
				result.substring(result.indexOf("web") + 6, result.length() - 2)
